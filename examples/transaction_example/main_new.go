...
func (s *ProductService) CreateCategoryWithProducts(ctx context.Context, categoryName string, products []*Product) error {
	...
	uow := postgres.NewUnitOfWork[*Category](s.db)
	...
	// Get the repository from Unit of Work
	categoryRepo := uow.GetRepository()
	...
	// 2. Create products with the category ID using direct DB access
	// Since we need a different entity type, we'll use the DB directly within the transaction
	db := categoryRepo.(*postgres.BaseRepository).GetDB()
...
func (s *ProductService) TransferStock(ctx context.Context, fromProductID, toProductID int, quantity int) error {
	// Create Unit of Work for product operations
	uow := postgres.NewUnitOfWork[*Product](s.db)
	...
	db := repo.(*postgres.BaseRepository).GetDB()

	// Get source product
	var fromProduct Product
	if err := db.WithContext(ctx).First(&fromProduct, fromProductID).Error; err != nil {
		uow.RollbackTransaction(ctx)
	...
	if err := db.WithContext(ctx).First(&toProduct, toProductID).Error; err != nil {
		uow.RollbackTransaction(ctx)
		return fmt.Errorf("failed to get target product: %w", err)
	}

	// Update stocks
	if err := db.WithContext(ctx).Model(&fromProduct).Update("stock", fromProduct.Stock-quantity).Error; err != nil {
		uow.RollbackTransaction(ctx)
		return fmt.Errorf("failed to update source stock: %w", err)
	}

	if err := db.WithContext(ctx).Model(&toProduct).Update("stock", toProduct.Stock+quantity).Error; err != nil {
		uow.RollbackTransaction(ctx)
